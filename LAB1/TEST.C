...
void  Task1 (void *pdata)
{
    ...
    while(1){
        if(OSTimeGet() >= 500){
            ...
            int i;
            for(i=0; i<500; i++){
                printf("%3d\t", OSTCBCur->ptr_struct[i].TimeTick);
                if(OSTCBCur->ptr_struct[i].event==1)
                    printf("%10s", "Preempt\t");
                else    
                    printf("%10s", "Complete\t");
                printf("%10d\t%10d\n", OSTCBCur->ptr_struct[i].FromTaskPrio, OSTCBCur->ptr_struct[i].ToTaskPrio);   
                if(i % 6 == 5){
                    printf("task3 deadline missed!\n");
                }
            }
        }
...
void  Task2 (void *pdata)
{
    ...
    OSTCBCur->period = 6;
    while(1){
        // if(OSTimeGet()<500 && OSTCBCur->PrintBuf.TimeTick){
        //     // printf("Task%d ",2);
