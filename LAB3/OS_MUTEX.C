    ...
    if ((INT8U)(pevent->OSEventCnt & OS_MUTEX_KEEP_LOWER_8) == OS_MUTEX_AVAILABLE) {
        ...
        if(print_log_iter < PRINT_LOG_NUMS){
            if(pip == 1){ /* R1 */
                ...
                LOG[print_log_iter][1] = 1; /* R1 */
                LOG[print_log_iter][2] = OSPrioCur;
                LOG[print_log_iter][3] = pip;
                LOG[print_log_iter++][4] = -2; /* -1: c/p log, -2,-3: block log, >=0: deadline miss log */
            }else if(pip == 2){ /* R2 */
                LOG[print_log_iter][0] = OSTime;
                LOG[print_log_iter][1] = 2; /* R2 */
                LOG[print_log_iter][2] = OSPrioCur;
                LOG[print_log_iter][3] = (OSPrioCur < pip) ? 1 : pip;
                LOG[print_log_iter++][4] = -2; /* -1: c/p log, -2,-3: block log, >=0: deadline miss log */
            }
        }
        //################## modified ##################//
...
INT8U  OSMutexPost (OS_EVENT *pevent)
{
    ...
    if(print_log_iter < PRINT_LOG_NUMS){
        if(pip == 1){ /* R1 */
            LOG[print_log_iter][0] = OSTime;
            LOG[print_log_iter][1] = 1; /* R1 */
            LOG[print_log_iter][2] = pip;
            LOG[print_log_iter][3] = prio;
            LOG[print_log_iter++][4] = -3; /* -1: c/p log, -2,-3: block log, >=0: deadline miss log */
            mutex_owner = (prio != 1 && prio != 2) ? prio : -1;
        }else if(pip == 2){ /* R2 */
            LOG[print_log_iter][0] = OSTime;
            LOG[print_log_iter][1] = 2; /* R2 */
            LOG[print_log_iter][2] = (prio < pip) ? 1 : pip;
            LOG[print_log_iter][3] = prio;
            LOG[print_log_iter++][4] = -3; /* -1: c/p log, -2,-3: block log, >=0: deadline miss log */
            mutex_owner = (prio != 1 && prio != 2) ? prio : -1;
        }
